...
// Store which nodes have been discovered.
nodesDiscovered = {
    'xbee0': false,
    'xbee1': false,
    'xbee2': false,
...
// Do a Node Discovery, and when they are all found return a promise.
xbee.prototype.WSNNodeDiscovery = function(){
    var xbeeAPI = this.xbeeAPI;
    var that = this;
    var countNodesDiscovered = 0;
    var totalNodes = Object.keys(addrXbee64).length;   // Total xbee devices or nodes.
    var maxTimeWait = 15000;    // Max time waiting for discover all nodes.
    ...
    .then(function(frame){
        ...
        console.log('Nodes not discovered are: ' + that.searchNodesNotDiscovered());
    }).done();
        ...
        function callback(receivedFrame){
            ...
            if(cmd === 'nd' || cmd === 'ND'){
                ...
                if(nodesDiscovered[xbeeKey] === false){
                    ...
                    addrXbee16[xbeeKey] = receivedFrame.remote16;   // Save 16bit address.
                    console.log(xbeeKey + ' found. Number of nodes discovered: ' + countNodesDiscovered + '/' + totalNodes);
                }
                // When all node have been discovered resolve the promise.
                if(totalNodes === countNodesDiscovered){
                    xbeeAPI.removeListener("frame_object", callback);   // xbeeAPI.on() will stop listening.
                    clearInterval(NDTimeout);
                    deferred.resolve(receivedFrame);    // Resolve the promise.
                }
            }
        };

        NDTimeout = setInterval(function(){
            // Stop trying to discover all node in network after 'totalNDTry' times.
            if(countNDTry === totalNDTry){
                xbeeAPI.removeListener("frame_object", callback);   // xbeeAPI.on() will stop listening.
                clearInterval(NDTimeout);
                deferred.reject(new Error('Could not discover all nodes in the network.'));
                return;
            }
            
            console.log('Retry ND broadcasting.');
            // Keep trying to discover all nodes in network.
...
xbee.prototype.getXbeeKeyByAddress64 = function(address){
    for(var xbeeKey in addrXbee64){
        if(addrXbee64[xbeeKey] === address) return xbeeKey;
    }
};

// Retrieve xbee key based on address: '2143' --> 'xbee1'
xbee.prototype.getXbeeKeyByAddress16 = function(address){
    for(var xbeeKey in addrXbee16){
        if(addrXbee16[xbeeKey] === address) return xbeeKey;
    }
};
